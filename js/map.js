import {collectSimilarOffer} from './data.js';
...
let mapBox = L.map('map-canvas')
  .on('load', () => {/*загрузка карты переводит формы в активное состояние*/
...
let collect = collectSimilarOffer();
...
collect.forEach((elements) => {
  ...
  pin
    .addTo(mapBox)
    .bindPopup(similarOffer(elements),
      {
        keepInView: true,
      },
    );
